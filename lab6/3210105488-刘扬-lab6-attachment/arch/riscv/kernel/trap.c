...
void do_page_fault(struct pt_regs *regs, unsigned long stval) {
    ...
    char *new_addr = alloc_page();
    create_mapping(current->pgd, PGROUNDDOWN(stval), (uint64)new_addr-PA2VA_OFFSET, PGSIZE, (vma->vm_flags | 0x11));
    if(!(vma->vm_flags & VM_ANONYM)){ //拷贝非匿名page
        ...
        if(stval - PGROUNDDOWN(vma->vm_start) < PGSIZE){
            ...
            uint64 sz = PGSIZE - offset;
            for(int j = 0;j < sz;j++){
                new_addr[j + offset] = src_addr[j];
            }
        }
        else{
            ...
            uint64 sz = PGSIZE;
            for(int j = 0;j < sz;j++){
                new_addr[j] = src_addr[j + PGSIZE * pg_offset - offset];
            }
        }
    }  
}